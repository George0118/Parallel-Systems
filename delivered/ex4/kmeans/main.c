...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if (rank == 0) {
        ...
        for (i = 0; i < size; i++) {
            if (i < size - 1) {
                ...
                recvcounts[i] = numObjs - (size - 1) * (rank_numObjs);
            }
        }
    ...
    MPI_Bcast(&recvcounts[0], size, MPI_INT, 0, MPI_COMM_WORLD);
     ...
     */
    printf("Hello this is process with rank %d\n", rank);
    MPI_Gatherv(membership, rank_numObjs, MPI_INT, tot_membership, &recvcounts[0], &displs[0], MPI_INT, 0, MPI_COMM_WORLD);
    ...
    printf("Hello this is process with rank %d and i am about to start freeing.\n", rank);
