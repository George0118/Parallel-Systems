...
int main(int argc, char ** argv) {
    ...
    MPI_Scatterv(Ustart, scattercounts, scatteroffset, global_block, &(u_previous[1][1]), 1, local_block, 0, MPI_COMM_WORLD);
    MPI_Scatterv(Ustart, scattercounts, scatteroffset, global_block, &(u_current[1][1]), 1, local_block, 0, MPI_COMM_WORLD);
    ...
    MPI_Request before_requests[6];
    MPI_Status before_status[6];
    MPI_Request after_requests[2];
    ...
    for (t=0;t<T;t++) {
        ...
        if (north != MPI_PROC_NULL){
            MPI_Isend(&u_previous[1][1], 1, local_horizontal_edge, north, 0, MPI_COMM_WORLD, &before_requests[requests_length]);
            MPI_Irecv(&u_current[0][1], 1, local_horizontal_edge, north, 0, MPI_COMM_WORLD, &before_requests[requests_length + 1]);
            requests_length += 2;
        }

        if (south != MPI_PROC_NULL){
            MPI_Irecv(&u_previous[local[0]+1][1], 1, local_horizontal_edge, south, 0, MPI_COMM_WORLD, &before_requests[requests_length]);
            requests_length++;
        }
        
        if (east != MPI_PROC_NULL){
            MPI_Irecv(&u_previous[1][local[1]+1], 1, local_vertical_edge, east, 0, MPI_COMM_WORLD, &before_requests[requests_length]);
            requests_length++;
        }

        if (west != MPI_PROC_NULL){
            MPI_Isend(&u_previous[1][1], 1, local_vertical_edge, west, 0, MPI_COMM_WORLD, &before_requests[requests_length]);
            MPI_Irecv(&u_current[1][0], 1, local_vertical_edge, west, 0, MPI_COMM_WORLD, &before_requests[requests_length + 1]);
            requests_length += 2;
        }

        MPI_Waitall(requests_length, before_requests, before_status);
        ...
        if (south != MPI_PROC_NULL){
            MPI_Isend(&u_current[local[0]][1], 1, local_horizontal_edge, south, 0, MPI_COMM_WORLD, &after_requests[requests_length]);
            requests_length++;
        }

        if (east != MPI_PROC_NULL){
            MPI_Isend(&u_current[1][local[1]], 1, local_vertical_edge, east, 0, MPI_COMM_WORLD, &after_requests[requests_length]);
            requests_length++;
        }

        MPI_Waitall(requests_length, after_requests, after_status);
