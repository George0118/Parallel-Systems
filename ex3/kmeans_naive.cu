                                                ...
                                                int blockSize)
{
    ...
    double total_cpu_timing = 0, cpu_timing;
    double total_gpu_timing = 0, gpu_timing;
    double total_transfer_timing = 0, transfer_timing;
    ...
    do {
        ...
        transfer_timing = wtime();      // start transfer timer
        ...
        transfer_timing = wtime() - transfer_timing;
        total_transfer_timing += transfer_timing;     // add to total transfer time

                //printf("Launching find_nearest_cluster Kernel with grid_size = %d, block_size = %d, shared_mem = %d KB\n", numClusterBlocks, numThreadsPerClusterBlock, clusterBlockSharedDataSize/1000);

        gpu_timing = wtime();       // start gpu timer
        ...
        total_gpu_timing += gpu_timing;       // add to total gpu time
        ...
        transfer_timing = wtime();      // start transfer timer again
        ...
        cpu_timing = wtime();   // start cpu timer
        ...
        total_cpu_timing += cpu_timing;       // add to total cpu time
        
    } while (delta > threshold && loop < loop_threshold);

    timing = wtime() - timing;

    printf("GPU Time: %lf\nCPU - GPU Transfers: %lf\nCPU Time: %lf\n", 1000*total_gpu_timing, 1000*total_transfer_timing, 1000*total_cpu_timing);
