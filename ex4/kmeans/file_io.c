...
double * dataset_generation(int numObjs, int numCoords, long *rank_numObjs)
{
    ...
    *rank_numObjs = numObjs / size;
    ...
    if (rank == 0) {
        ...
        for (i = 0; i < size; i++) {
            if (i < size - 1) {
                ...
                sendcounts[i] = (numObjs - (size - 1) * (*rank_numObjs)) * numCoords;
            }
        }
    ...
    MPI_Scatterv(objects, &sendcounts[0], &displs[0], MPI_DOUBLE, rank_objects, (*rank_numObjs) * numCoords, MPI_DOUBLE, 0, MPI_COMM_WORLD);
